...
const fieldFactory = _.flow(
  _.pick(['name', 'alias', 'subfields', 'fragments', 'args']),
...
const setAlias = alias => field => fieldFactory({ ...field, alias });
...
const getFieldRepr = (field) => {
  const { name, alias, args, subfields, fragments } = field;
  let repr = alias ? `${alias}:${name}` : name;
  if (!_.isEmpty(args)) {
